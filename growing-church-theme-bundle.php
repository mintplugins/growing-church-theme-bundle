...
if( !defined( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_URL' ) )
	define( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

// Plugin Folder Path
if( !defined( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR' ) )
	define( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );

// Plugin Root File
if( !defined( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_FILE' ) )
	define( 'GROWING_CHURCH_THEME_BUNDLE_PLUGIN_FILE', __FILE__ );
...
function growing_church_theme_bundle_textdomain() {

	// Set filter for plugin's languages directory
	$growing_church_theme_bundle_lang_dir = dirname( plugin_basename( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_FILE ) ) . '/languages/';
	$growing_church_theme_bundle_lang_dir = apply_filters( 'growing_church_theme_bundle_languages_directory', $growing_church_theme_bundle_lang_dir );


	// Traditional WordPress plugin locale filter
	$locale        = apply_filters( 'plugin_locale',  get_locale(), 'growing-church-theme-bundle' );
	$mofile        = sprintf( '%1$s-%2$s.mo', 'growing-church-theme-bundle', $locale );

	// Setup paths to current locale file
	$mofile_local  = $growing_church_theme_bundle_lang_dir . $mofile;
	$mofile_global = WP_LANG_DIR . '/growing-church-theme-bundle/' . $mofile;

	if ( file_exists( $mofile_global ) ) {
		// Look in global /wp-content/languages/growing_church_theme_bundle folder
		load_textdomain( 'growing_church_theme_bundle', $mofile_global );
	} elseif ( file_exists( $mofile_local ) ) {
		// Look in local /wp-content/plugins/message_bar/languages/ folder
		load_textdomain( 'growing_church_theme_bundle', $mofile_local );
	} else {
		// Load the default language files
		load_plugin_textdomain( 'growing_church_theme_bundle', false, $growing_church_theme_bundle_lang_dir );
	}

}
add_action( 'init', 'growing_church_theme_bundle_textdomain', 1 );

/**
 * Activation Hook Function - Gets Stack Pack License, Redirects to installation of dependencies, saves Theme MetaData.
 */
require( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . 'includes/misc-functions/standard-install-functions.php' );
...
|--------------------------------------------------------------------------
*/
function growing_church_theme_bundle_include_files(){
	...
	if (!function_exists('mp_core_textdomain')){
		 ...
		 */
		require( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . '/includes/plugin-checker/class-plugin-checker.php' );
		
		/**
		 * Include Plugin Installer
		 */
		require( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . '/includes/plugin-checker/class-plugin-installer.php' );
		
		/**
		 * Check if mp_core in installed
		 */
		include_once( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . 'includes/plugin-checker/included-plugins/mp-core-check.php' );
	...
	//If mp_core IS Active
		...
		$plugin_vars = array(
			'plugin_name' => 'Growing Church Theme Bundle',
		...
		if( $license_key_valid != true ){
			 ...
			 */	
			new MP_CORE_Show_License_Form_In_Notices( array('plugin_name' => 'Growing Church Theme Bundle' ) );
			
			/**
			 * Update script - keeps this plugin up to date
			 */
			require( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . 'includes/updater/growing-church-theme-bundle-update.php' );
			
				
		}
		//If MP Stacks or Knapstack aren't installed
		elseif( !growing_church_theme_bundle_dependencies() ) {
			...
			foreach( growing_church_theme_bundle_dependencies_array() as $function_exist_name => $checker_file_name ){
				
				/**
				 * Check the status of the required plugin and install it if not. Activate it if it is inactive
				 */
				require( GROWING_CHURCH_THEME_BUNDLE_PLUGIN_DIR . 'includes/plugin-checker/included-plugins/' . $checker_file_name ); 
...
add_action('after_setup_theme', 'growing_church_theme_bundle_include_files', 9);
...
function growing_church_theme_bundle_dependencies(){
	...
	$required_text_domain_strings = growing_church_theme_bundle_dependencies_array();
...
function growing_church_theme_bundle_dependencies_array(){
	return array(
		//Hard Coded Stuff:
